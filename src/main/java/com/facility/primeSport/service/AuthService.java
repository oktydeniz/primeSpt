...
import com.facility.primeSport.auth.TokenManager;
...
import com.facility.primeSport.util.form.VerifyFormInput;
...
public class AuthService {
    ...
    public ResponseEntity<AuthResponse> register(UserAuthRegisterRequest request) {
        AuthResponse authResponse = new AuthResponse();
    ...
    public String registerForToken(UserAuthRegisterRequest request) {
        if (userService.getUserByEmail(request.getEmail()) != null) {
    ...
    public String loginForToken(UserAuthRegisterRequest request) {
        UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(request.getEmail(), request.getPassword());
        Authentication authentication = authenticationManager.authenticate(token);
        SecurityContextHolder.getContext().setAuthentication(authentication);
        return "Bearer " + tokenManager.generateToken(authentication);
